...
random.seed(42)
np.random.seed(42)
OUTPUT_DIR = Path(__file__).resolve().parents[2] / "data" / "datasets"
...
# Save datasets
df.to_csv(OUTPUT_DIR / 'complete_student_dataset_9000.csv', index=False)
train_df.to_csv(OUTPUT_DIR / 'train_dataset.csv', index=False)
val_df.to_csv(OUTPUT_DIR / 'validation_dataset.csv', index=False)
